...
class WhisperTranscriber:
    ...
    def __init__(self, logger_callback=None):
        ...
        if self.device == "cuda":
            ...
            self.logger(f"   Тип данных: FP16 (ускорение)", "info")
        else:
    ...
    def _save_docx(self, result: Dict, output_path: Path, filename: str):
        try:
            ...
            self.logger("⚠️ python-docx не установлен. Сохранено в TXT", "warning")
...
<head>
    ...
    <style>
        ...
        .timestamp {{ color: #4CAF50; font-weight: bold; }}
    </style>
</head>
<body>
    <h1>Транскрибация: {filename}</h1>
"""
        for segment in result['segments']:
            html_content += f"""
    <div class="segment">
        <div class="timestamp">[{self._format_time_srt(segment['start'])}]</div>
        <div>{segment['text'].strip()}</div>
    </div>
"""
    ...
    def _save_csv(self, result: Dict, output_path: Path):
        ...
        with open(output_path, 'w', encoding='utf-8', newline='') as f:
            ...
            for i, segment in enumerate(result['segments'], 1):
                writer.writerow([i, self._format_time_srt(segment['start']), 
                                self._format_time_srt(segment['end']), segment['text'].strip()])
    ...
    def transcribe_all(self, cache_dir: Path, text_dir: Path, 
        ...
        for idx, wav_path in enumerate(wav_files, 1):
            ...
            if output_path.exists() and not force_overwrite:
                self.logger(f"⏭️  Пропущен (уже есть): {wav_path.name}", "warning")
