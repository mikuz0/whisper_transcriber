...
class Validators:
    """Различные проверки (только предупреждения, без запретов)"""
    ...
    def check_ram_available() -> Tuple[float, str]:
        """
        Проверяет доступную RAM и возвращает предупреждение при необходимости
        
        Returns:
            (available_gb, warning_message)
        """
        try:
            import psutil
            ...
            if available_ram < 2:
                warning = f"⚠️  Доступно всего {available_ram:.1f} ГБ RAM. "
                warning += "Для длинных файлов может не хватить памяти."
            elif available_ram < 4:
                warning = f"⚠️  Доступно {available_ram:.1f} ГБ RAM. "
                warning += "Рекомендуется использовать модель 'tiny' или 'base'."
    ...
    def check_model_requirements(model_name: str) -> Tuple[bool, str]:
        """
        Проверяет, достаточно ли RAM для выбранной модели (только предупреждение)
        ...
        try:
            ...
            if available_ram < required:
                warning = f"⚠️  Модель {model_name} требует ~{required} ГБ RAM, "
                warning += f"доступно {available_ram:.1f} ГБ. Возможны проблемы."
    ...
    def check_file_duration(duration_seconds: float, model_name: str = 'base') -> str:
        """
        Возвращает предупреждение о длительности файла
        
        Returns:
            warning_message (может быть пустой)
        """
    ...
    def ensure_directories(work_dir: Path) -> Tuple[Path, Path, Path, Path]:
        """
        Создает необходимые папки в рабочей директории
        
        Returns:
            (video_audio_dir, audio_cache_dir, text_dir, text_processed_dir)
        """
    ...
    def validate_work_directory(work_dir: Path) -> Tuple[bool, str]:
        """
        Проверяет, что рабочая директория доступна для чтения/записи
